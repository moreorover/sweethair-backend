...
export const all = async (req: Request, res: Response) => {
    ...
    const results = await service.all({ relations: ['appointments', 'transactions', 'items'], order: { fullName: 'ASC' } });
...
export const paginate = async (req: Request, res: Response) => {
    ...
    const result = await service.paginate(page, { relations: ['appointments', 'transactions', 'items'], order: { fullName: 'ASC' } });
    return res.send(result);
};

export const findById = async (req: Request, res: Response) => {
    const service: CustomerService = new CustomerService(Customer);
    const results = await service.findOne({ id: parseInt(req.params.id) }, { relations: ['appointments', 'transactions', 'items'] });
...
export const update = async (req: Request, res: Response) => {
    ...
    const savedCustomer = await service.findOne({ id: parseInt(req.params.id) }, { relations: ['appointments', 'transactions', 'items'] });
    return res.send(savedCustomer);
};
